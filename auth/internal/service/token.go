...
func (s *TokenService) GenerateTokens(
	...
	accessToken, err := s.generateAccessToken(user.GetId(), userType, protoRoles)
	if err != nil {
...
func (s *TokenService) generateAccessToken(
	...
	token, err := s.jwtManager.Generate(userID, userType, roleStrings)
	if err != nil {
		return "", fmt.Errorf("%w (%w)", domain.ErrInternal, err)
	}
